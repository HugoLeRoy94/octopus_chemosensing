...
* **Source Assignment:** Each ligand belongs to one fixed presence block, assigned independently of its chemical family. Presence is sampled hierarchically as described below, rather than by independent per-ligand Bernoulli draws.
...
1. **The Mixture Mask:** Draw a positive source count from a zero-truncated Poisson law, additionally truncated at the number of source blocks. Select that many blocks uniformly without replacement. Within each selected block, draw a positive ligand count from the similarly truncated law and choose those ligands uniformly without replacement. The resulting multi-hot mask $M$ is never empty. The rate parameters are `mu_sources` and `mu_ligands_per_source`; after truncation they are not the mean counts. See §07 stage 2 for implementation details.
...
The implementation stores selected ligands in a padded sparse table. Its usual width
`s_upper` is a tail-based allocation heuristic, not a limit on the sampled mixture.
If any sniff exceeds it, the table widens for that batch to retain every selected
ligand. Presence labels and concentrations used by receptor physics therefore describe
the same mixture, including rare large draws.
