...
\subsection{Disentangling Information: Ligand Identity vs. Concentration}

While the total mutual information $I(\mathcal{A};(c,\ell))$ provides a global metric for the array's coding capacity, a biological system such as an octopus may not optimize its sensory apparatus to maximize this total information blindly. Depending on ecological pressures, accurately identifying the chemical nature of a ligand (its family) might carry a vastly different biological value than precisely resolving its environmental concentration $c$. To account for these distinct evolutionary objectives, we decompose the total mutual information to independently measure the array's ability to encode ligand identity versus concentration. 


We define the ligand family as a discrete variable $F$, grouping distinct chemicals into generic probability clouds. The mutual information between the array's activity $\mathcal{A}$ and the ligand family $F$ is given by:
\begin{equation}
I(\mathcal{A}; F) = H(\mathcal{A}) - H(\mathcal{A} | F)
\end{equation}
Here, $H(\mathcal{A})$ is the unconditioned Shannon entropy of the array's responses. The conditional entropy $H(\mathcal{A} | F)$ represents the remaining uncertainty in the array's activity when the ligand family is known. Computationally, this is evaluated by masking the continuous relaxation assignments to isolate specific families, computing the entropy for each, and averaging across all unique families present in the evaluation batch. 

Similarly, we can isolate the information the array encodes specifically about the ligand's concentration $c$:
\begin{equation}I(\mathcal{A}; c) = H(\mathcal{A}) - H(\mathcal{A} | c)\end{equation}
Because the concentration $c$ is drawn from a continuous Log-Normal distribution, calculating the conditional entropy $H(\mathcal{A} | c)$ requires discretization. We achieve this by sorting the environmental batch by concentration and partitioning it into $N_c$ equally sized bins. The conditional entropy is then approximated as the average joint entropy of the array's activity within each discrete concentration bin. 

By tracking these two metrics independently, we can explicitly test whether combinatorial heteromerization offers a specific coding advantage for distinguishing between distinct chemical families, or if it primarily serves as a mechanism to resolve concentration gradients across a vast dynamic range.
...
\begin{figure}
    \includegraphics[width=\linewidth]{../figures/entropy_distance.pdf}
...
\begin{figure}
    \includegraphics[width=0.45\textwidth]{../figures/1fam_5sensor_homo.pdf}
...
%\end{figure*}
\begin{figure}[!htbp]
    \includegraphics[width=0.45\textwidth]{../figures/MI_sensors_1fam.pdf}
...
\begin{figure}
        \includegraphics[width=0.45\textwidth]{../figures/10fam_5sensor.pdf}
...
\begin{figure}
        \includegraphics[width=0.45\textwidth]{../figures/MI_sensors_10fam.pdf}
